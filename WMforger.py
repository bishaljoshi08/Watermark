...
TEMP_OUT_DIR = Path("/scratch/bjoshi/watermark/submission_temp_wmforger_v2")
FILE_PATH    = "/scratch/bjoshi/watermark/submission_wmforger_v2.zip"
...
# Optimization (official defaults from optimize_image.py)
NUM_STEPS = 50
LR        = 0.05
...
def main():
    ...
    for wm, start, stop in CATEGORIES:
        ...
        # --- Extract + average the watermark across all sources ---------
        # Extract each at its native size, resize to a common cat_size, then
        # average. Averaging reduces the residual content component that
        # doesn't cancel in a single-shot extraction.
        wm_accum = None
        for i, sp in enumerate(src_paths):
            src_pil = Image.open(sp).convert("RGB")
            w_native = extract_watermark_from_source(src_pil, model)
            w_at_cat = resize_delta(w_native, cat_size)
            wm_accum = w_at_cat if wm_accum is None else wm_accum + w_at_cat
            print(f"    extracted {i+1}/{len(src_paths)} "
                  f"(|w| mean={np.abs(w_native).mean():.3f})")
        watermark = wm_accum / len(src_paths)
        print(f"  averaged watermark |w| mean = {np.abs(watermark).mean():.3f}")
        ...
        for tp in grp_targets:
            ...
            d = resize_delta(watermark, (clean.shape[1], clean.shape[0]))
            if ENABLE_MASKING:
