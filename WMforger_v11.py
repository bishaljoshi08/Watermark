...
TEMP_OUT_DIR = Path("/scratch/bjoshi/watermark/submission_temp_wmforger_v11")
FILE_PATH    = "/scratch/bjoshi/watermark/submission_wmforger_v11.zip"
...
NUM_STEPS = 200   # confirmed: 150 recovered WM_8 and gave 0.963 overall Sqlt
LR        = 0.05


ALPHA = 3   # default for the strong categories
ALPHA_PER_CATEGORY = {
    "WM_6": 1.5,   # weakest category - don't push; rely on masking instead
}

EXTRACTION_MODE = "median"


MASK_CATEGORIES = {"WM_6", "WM_1", "WM_2", "WM_3", "WM_4", "WM_5", "WM_7", "WM_8"}   # set() to disable everywhere; add names to extend
...
def main():
    ...
    for wm, start, stop in CATEGORIES:
        ...
        use_mask = wm in MASK_CATEGORIES
        if use_mask:
            print(f"  masking ON for {wm} (smooth-region attenuation)")
